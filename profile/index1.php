...
// Получение заказов пользователей
$userOrders = $db->query("
    SELECT wallpaper.name_wall AS 'name_wall', 
           wallpaper.style_wall AS 'style_wall', 
           orders.quantity AS 'quantity', 
           user.fullname AS 'fullname', 
           wallpaper.price AS 'price', 
           (orders.quantity * wallpaper.price) AS 'items_price'
    FROM orders 
    JOIN wallpaper ON orders.id_wallpaper = wallpaper.id
...
<body>
    ...
    <div style="text-align: center; margin-top: 30px; ">
        <div class="but"><button onclick="location.href='../profile/index.php';">Просмотр всех обращений</button></div>
    </div>
    ...
    <?php foreach ($userOrders as $order): ?>
        <tr>
            <td><?= htmlspecialchars($order['name_wall']) ?></td>
            <td><?= htmlspecialchars($order['style_wall']) ?></td>
            <td><?= htmlspecialchars($order['quantity']) ?></td>
            <td><?= htmlspecialchars($order['fullname']) ?></td>
            <td><?= htmlspecialchars($order['price']) ?></td>
            <td><?= htmlspecialchars($order['items_price']) ?></td>
        </tr>
    <?php endforeach; ?>
</table>
    </div>
